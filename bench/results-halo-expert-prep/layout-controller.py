...
UNIT='qk-native-campaign-expert-layout-tAFAFc'
...
def main():
    ...
    for mode in ('baseline','words','soa'):
        subprocess.run(['spirv-val','--target-env','vulkan1.2',str(OUT/(mode+'.spv'))],check=True)
    files=[Path(__file__),OUT/'watch.py',OUT/'operator',ROOT/'tests/halo_expert_layout.cpp',
           ROOT/'tests/halo_expert_layout.comp',ROOT/'tests/halo_gemm_operator_common.h',
           Path('/usr/lib/x86_64-linux-gnu/libvulkan_radeon.so')]+[OUT/(s+'.spv') for s in ('baseline','words','soa')]
    ...
    try:
        ...
        expected=[(layer,a,n,scale,mode,i) for layer in (0,47) for a,n,scale in cases
                  for i,mode in enumerate(('baseline','words','soa','soa','words','baseline'))]
