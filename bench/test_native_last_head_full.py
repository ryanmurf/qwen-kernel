...
def http_fixture(mode):
    ...
    metadata=dict(mode=mode,http_returncode=0,actual_environment={'QK_FLASH_PREFILL_LAST':'1' if mode=='last' else '0'},
        ...
        for repetition in (1,2,3):
            for kind in ('prefill','decode'):
                rows.append(dict(type=kind,**common,prompt_tokens=size,repetition=repetition,
                    ...
                    streamed_tokens=128,ttft_seconds=1.,decode_tokens_per_second=32.,
...
class FullLastHeadTest(unittest.TestCase):
    ...
    def test_gate_rejects_corruption(self):
        for mutation in ('missing_row','missing_case','hash','base','flag','result','unclosed','greedy','environment'):
