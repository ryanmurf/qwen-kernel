...
static bool caseQ6K(VkCtx& c, uint32_t M, uint32_t K, uint32_t iters) {
    ...
    // QK_Q6K_SHADER=gemv_q6_k_v1.spv benchmarks the byte-addressed original.
    const char* q6kShader = getenv("QK_Q6K_SHADER");
    return runGemv(c, q6kShader ? q6kShader : "gemv_q6_k.spv", blocks.data(), nb * sizeof(block_q6_K),
                   x, M, K, yref, iters, K / 32);
}
...
bool qk_engine::open(const char* path, const qk_config& cfg, char* err, size_t errLen) {
    ...
    if (arch == "qwen4exp") {
        ...
        fprintf(stderr,"native Flash prefill GEMM policy: %s\n",qwen4->prefillGemm());
        fprintf(stderr,"native Flash prefill attention policy: %s\n",qwen4->prefillAttention());
        return true;
    }
