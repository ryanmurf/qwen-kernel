...
static bool caseQwen4Hc(VkCtx& c, uint32_t n, uint32_t h, uint32_t tokens) {
    if (!n || n > 16384 || !h || h > 16 || !tokens || tokens > 32) return false;
    ...
    bool pass = true;
    for (pc.mode = 0; pc.mode < 3; ++pc.mode) {
        ...
        if (pc.mode == 0) {
            for (uint32_t t = 0; t < tokens; ++t) for (uint32_t s = 0; s < h; ++s) {
                ...
                for (uint32_t i = 0; i < n; ++i)
                    reference[base+i] = host[0][base+i] * scale * host[1][s*n+i];
            }
        } else if (pc.mode == 1) {
        ...
        uint32_t gx = pc.mode == 0 ? h : ((pc.mode == 1 ? n : n*h) + 255) / 256;
