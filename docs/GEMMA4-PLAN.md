...
Specialize those three inner dimensions so trip counts and tails are compile-time constants. Load each packed word and scale once, process its paired low/high nibbles, and reduce in the subgroup. Sweep 32/64/128 threads per row and local sizes 128/256; the table is a starting point, not a hard-coded policy.

For prefill and speculative verification, add weight-stationary Q4_0 GEMM. Start with `BM=128, BN=64, BK=64` and retain `BN=32` for small batches. A simple f32 LDS stage uses `BK*BN*4 + BM*BK*4 = 48 KiB` at 128/64/64, so double buffering requires narrower tiles or packed-weight staging. Decode and GEMM must share the exact dequant routine. At pp512, active matrix arithmetic is about `2*3.822B = 7.64 GFLOP/token`; 8–12 TFLOP/s useful throughput implies 1.05–1.57K prompt tok/s before attention overhead.

The 1.728 GB Q4 portion costs 1.92–2.03 ms at 900–850 GB/s, only 0.12–0.23 ms above its nominal 960 GB/s floor. Simpler dequant should buy lower VALU/VGPR pressure than IQ4_XS and thus higher achieved bandwidth, but not fewer bytes.
...
### 2.3 Routed experts: two dispatches, not 24

One expert matrix is `2816*704 = 1,982,464` weights or 1,115,136 Q4 bytes (1.064 MiB). Its ideal 960 GB/s read lasts 1.16 microseconds, below qk's observed roughly 1.4 microsecond stage floor. Twenty-four independent matrix dispatches per layer therefore spend at least 33.6 microseconds/layer on floors alone and never settle into the 90–97% bandwidth regime.

Keep the useful shape of qk's `moe_gateup_*`/`moe_down_*` design: one combined gate+up dispatch and one down dispatch per layer. Use x for output-row tiles, y for the eight selected-expert ranks, and z for serving/batch slot; fetch the actual expert ID indirectly from `SelT`.

A single gate/up expert has 704 output rows. Four rows per 256-thread WG gives 176 WGs, 1,408 wave32-equivalents, only 14.7 waves/CU over 96 CUs. It can touch every CU, but the whole 1.064 MiB read lasts ~1.16 microseconds and has almost no ramp/tail tolerance. Combining eight experts and both gate/up projections gives 1,408 WGs, 11,264 waves, or 117 waves/CU while reading 17.84 MB/layer. Target at least two resident 256-thread WGs/CU without spills; compare a 128-thread/two-row version if VGPR occupancy fails.

Down launches one 64-thread WG per 2816 output row and loops over all eight selected expert activations. That is 2,816 WGs, 5,632 waves, or 58.7 waves/CU, reading 8.92 MB/layer. It reduces weighted expert outputs in-register and writes one vector.

The pair reads 26.76 MB/layer, whose nominal time is 27.9 microseconds. Small regions and reductions make 90–97% unrealistic; target 70–80% nominal for gate+up, 55–70% for down, and 65–75% combined. Across 30 layers this predicts **1.07–1.34 ms, point 1.12 ms**, versus the 0.836 ms nominal floor.
...
Unique KV reads are 115.3 MB at depth 512, 230.7 MB at 1K, and 880.8 MB at 32K. The short-context budget uses **0.13 ms at depth 512**. A naive per-Q-head shader adds about 0.36 GB at 1K and 4.91 GB at 32K, moving the 32K nominal roof from 294 tok/s toward 118 tok/s.
...
Extend the existing Q6_K path with fused per-WG `(value,index)` maxima and a small final reduction. Apply forbidden multimodal-token bias as negative infinity during comparison. Gemma's `30*tanh(logit/30)` softcap is monotone, so top-1 need not evaluate it. Preserve llama.cpp's tie rule. Avoiding the 1 MiB materialized f32 logit vector saves only about 10–25 microseconds; it is exact, but not the main win. Predicted contribution: **0.78 ms**.

Requantizing only the head to Q4_0 saves 190.32 MB/token and raises the nominal roof from 403.4 to 438.4 tok/s, 8.7%. Reject it as the default: the head is tied to the embedding, the delivered QAT artifact deliberately retains Q6_K, and changing it can alter quality and exact IDs. IQ4_XS is no better. A cheaper head is a separately named model experiment, not an engine optimization.
...
The 0.918 value is a sensitivity, not an assumption. Measure acceptance on qk chat, prose, and coding fixtures. Start at K=4, then sweep 2/4/8. With 85–92% acceptance, batched verification, and a quantized assistant, predict **330–380 emitted tok/s**. Stop if acceptance is below 75%, assistant time exceeds 30% of a round, or verification degenerates into K serial GEMVs.

Gate: forced all-reject, partial-accept, and all-accept cases emit exactly the same IDs as target-only greedy decode.

### 2.8 Smaller siblings and local oracle

There is no smaller sparse A4B sibling. The fastest verified QAT Q4_0 bring-up target is [`google/gemma-4-E4B-it-qat-q4_0-gguf`](https://huggingface.co/google/gemma-4-E4B-it-qat-q4_0-gguf); [`google/gemma-4-12B-it-qat-q4_0-gguf`](https://huggingface.co/google/gemma-4-12B-it-qat-q4_0-gguf) is a larger dense cross-check. E2B, E4B, 12B, and 31B can validate loader, Q4, norms, and parts of attention, but none validates 128-expert A4B routing or serves as its drafter.
...
Gate: artifact ledger reconciles, both cards have repeatable bandwidth/baseline data, and llama.cpp produces stable reference IDs on two clean runs.
...
Reference: `/mnt/data/llama.cpp-master`, commit `a935fbffe1a3d31509c325c116454ab5d56b2eb8`. The current sandbox cannot enumerate Vulkan devices (`ggml_vulkan: No devices found`), so these commands must run directly on tron. Pin by PCI and verify that `Vulkan0` is the XTX:
...
"$BIN/llama-batched-bench" -m "$MODEL" -dev Vulkan0 -sm none -mg 0 -ngl 99 \
  ...
  -npp 512 -ntg 128 -npl 1,2,4,8,16 --output-format jsonl \
  > llama-gemma4-xtx-batched-independent.jsonl

"$BIN/llama-batched-bench" -m "$MODEL" -dev Vulkan0 -sm none -mg 0 -ngl 99 \
  -fa on -ctk f16 -ctv f16 -fit off -c 32768 -b 8192 -ub 512 \
  -npp 512 -ntg 128 -npl 1,2,4,8,16 -pps --output-format jsonl \
...
No valid target baseline has been measured in this sandbox. The planning expectation for llama.cpp commit a935fbf is **130 tok/s**, range **100–160**, for short-context single-stream tg. This is deliberately broad: qk's existing Qwen 35B result implies llama.cpp realizes roughly 180–210 GB/s of effective active-weight traffic, while Gemma's simpler Q4 dequant helps but its 1 MiB experts and 605 MB head hurt. Stage 0 replaces this inference with the same-day median.

The XTX short-context qk budget at depth about 512 is:
...
| Attention core at depth 512 | 115.34 MB KV | 0.13 ms | GQA reuse assumed |
...
Prediction: **270 tok/s on XTX**, plausible range **250–290**; **222 tok/s on XT**, range **210–235**. Against the provisional llama.cpp point this is 2.1x. The acceptance target is at least 1.3x the actual same-day llama.cpp median after exact parity. At 32K, KV traffic and split reduction move the XTX estimate to roughly **200–225 tok/s**.

The corrected nominal XTX roof is 403.4 tok/s (2.479 ms), so the central residual is 1.225 ms:

- about 0.29 ms from Q4/Q6 dequantization, block layout, and measured rather than nominal bandwidth;
- 0.20–0.25 ms from small-expert ramp, imbalance, and down reductions;
- 0.30–0.35 ms from routers, selectors, norms, epilogues, cache writes, and 30-layer dispatch floors;
- about 0.13 ms from depth-512 attention;
- 0.20–0.25 ms from activation traffic, barriers, argmax, and conservative rounding.

For pp512, the 7.64 GFLOP/token matrix load and grouped-expert efficiency imply **1.05–1.57K prompt tok/s** until measured. Exact B=4 batching should move aggregate throughput toward 400–500 tok/s as more weights become stationary. Optional MTP is predicted at 330–380 emitted tok/s if its acceptance and cost gates hold.
...
10. **Prefill estimate.** 8–12 useful TFLOP/s and grouped-expert balance are unmeasured; pp can land outside 1.05–1.57K.
11. **llama.cpp expectation.** 130 tok/s is an inference, not a result. Only the specified commit/host commands establish the comparison.
...
13. **Maximum-context memory.** Model plus KV leaves only ~4.0 GB decimal. Scratch, batching, duplicated tied weights, or an assistant can exhaust VRAM.
