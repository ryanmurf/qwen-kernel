...
**GQA refinement:** The strongest exact schedule is to share each K/V stream
across the 2/4/8 query heads that already map to it, while keeping a separate
softmax and original 256-key partial for every query head.  This supersedes
f32 producer coarsening for balanced work: full 35B B1/B8 at 8k improved about
16%/43%, B1 at 32k improved about 49%, and thermally matched B8/32k improved
about 2.1x.  The same default path improves 80B B8/8k about 32% and remains
bit-exact to the prior split schedule.
...
### 21. Optional f16 KV storage and decode GQA reuse — high deep-context EV

**Expected value:** About 1.8 GB saved in the measured configuration and roughly
2x KV capacity. Plain half storage was not faster, but format-specific grouped
query-head reuse can turn the smaller cache into a large decode-bandwidth win.
...
**Risk:** Known long-context greedy divergence; half storage alone had no speed
gain before the grouped-query kernel. Never default-on under the token-exact
contract.

**Decode refinement:** A later format-aware split kernel now loads each half K/V
stream once for 2/4/8 adjacent query heads while preserving an independent
softmax and original partial for every head.  This makes f16 materially faster:
full 35B B1/B8 at 8k improved about 18%/43%, and B1/B8 at 32k improved about
58%/2.23x, with hashes identical to the pre-refinement f16 path.  It remains an
opt-in precision tier because storage rounding—not this bit-exact schedule
change—can diverge from f32.
...
**Result:** The 32k/49k profile did expose a dominant KV bandwidth wall.  An
opt-in `QK_KV_Q8=1` tier now stores every logical 256-value K/V row as one
272-byte, 16-byte-aligned record (f32 scale plus 256 signed bytes), dequantizes
into f32, and shares each Q8 K/V stream across adaptive groups of 2/4/8 query
heads during split decode.  It is 3.76x smaller than f32 KV.  At 32k, full 35B
B1 decode fell from 20.771 to 10.918 ms/step and B8 from 139.031 to about
47 ms/step.  The tier is deterministic and self-consistent, but can diverge
from f32 greedy output after a long-context near tie; it therefore remains
explicitly lossy and default-off.  The result log records quality, memory,
prefill, and rejected-layout details.
...
**Result:** Retain an explicit 35B-only `QK_PREFILL_REPACK=1` capacity/speed
tradeoff. At open it transposes the owned Q8 dense projections from native
`[M][K/32]` GGUF rows into one 256-byte-aligned `[K/32][M]` arena; aligned
N-multiple-of-32 prefill uses the new kernel while decode and tail chunks keep
the native row-major tensors. The full model needs 1.27 GiB and 126-147 ms to
pack. Within-process real-shape brackets show about 4-7% lower dense-GEMM time,
projection-isolation falls about 3-4.5%, and five-repeat pp512 controls
324.34/325.46 ms fall to 321.61/322.86 ms, a conservative 0.5-1.2% end-to-end
gain. It stays default-off because this is too little to justify 1.27 GiB in
the production memory envelope. `QK_PREFILL_REPACK_USE=0` keeps the arena but
restores row-major dispatch for matched-memory A/B.

The broader layout/cache family is closed. The K-major Q8 kernel is
bit-for-bit identical to the aligned kernel on all real projection shapes,
but K-major decode mappings with 4/8/16 rows per SIMD were 23.2/27.3/31.1 us
versus 22.2 us row-major, so one representation cannot efficiently serve both
axes. IQ4 K-major prefill was neutral (roughly 0-1%) and is disabled on 80B.
Warm runtime compilation totals only 3.69 ms for 21 libraries plus 3.36 ms for
67 pipelines, compared with about 1.4 s unwired or 3.5 s wired process open;
this host also has no `metal`/`metallib` command-line compiler. A binary archive
or persistent versioned sidecar therefore cannot repay its invalidation and
deployment complexity here; Metal's own warm cache and the 147-ms runtime
transpose are sufficient for the opt-in path.
...
**Progress:** Complete. Results 25a and 25c land exact wins; the final audit
reverts 25b's unsafe crossover, and 25d closes the remaining DeltaNet geometry
with measured correctness and hardware walls.
...
**Progress:** Complete. The M4 Max exposes only `GPUTimestamp`, and only at
compute-encoder stage boundaries (`stage=1`, dispatch/draw/blit=0); occupancy,
byte, and dispatch-boundary counters are a hardware/API wall. `QK_COUNTERS=1`
therefore splits prefill into fenced labeled passes and calibrates each pass to
the command-buffer timestamp while explicitly warning that the trace is
perturbed. Normal execution retains one concurrent encoder. The checked-in
`scripts/perf_ledger.sh` refuses an unacknowledged live-worker run and records
...
| GPU-resident multi-step serving | A one-command-buffer token/position ring was exact with EOS disabled, but production K=2 moved only about 0.9% on 35B and 0-0.7% in the stable 80B bracket. Interior EOS advances recurrent state beyond the accepted prefix, so preserving `QK_PCACHE` requires a 62.8 MiB backup/replay or shader-wide masking. Keep per-step submission; the standalone token loop is already fully GPU-resident. |
...
| Decode-attention merged partials / tail-balanced fan-in | Merging several 256-key chunks into one softmax partial was much faster, but changes reduction grouping by about 1e-8 and flipped an 80B deep-prefix greedy trajectory. Retain one independent partial per original chunk. |
| Decode-attention producer/reducer atomic completion | The installed compiler does not expose Metal 4.1 acquire/release atomics. Device-fence plus relaxed-atomic variants were neutral in the best B8/8K case and slower elsewhere; keep the explicit reducer dispatch. |
| Router-logit/selector atomic completion | A device-fence plus relaxed completion counter passed synthetic comparisons, then produced a nondeterministic 80B B2 token trajectory in a repeated full-engine GPU A/B. Metal's available ordering primitives do not make ordinary logit stores safely consumable by the last TG; keep the explicit buffer barrier and selector dispatch. |
| Threadgroup-cached residual/RMS tail | Exact 64/128/256-thread virtual-lane variants and a just-written-device-buffer reload were tested. Cached-256 improved the isolated N512 tail about 26.0 -> 24.0 us, but pp512 and decode A/B were noise-flat; smaller geometries lost badly. The prototypes were removed. |
...
| Half DeltaNet matrix state | A f16-storage/f32-accumulate prototype saved only 30 MiB/slot on 35B (36 MiB/slot on 80B). It improved the isolated B8 row-state kernel 5.6-9.2%, but B1 still lost to the shipping split schedule, fast chunked pp512 did not improve, and exact per-token rounding forced the 4.23x-slower sequential path. Recurrent rounding compounded from step 8 and produced 5% worst serial-versus-chunked relative logit drift. Keep recurrent state f32. |
| Half normalized-activation mirror | Pre-rounded half input made isolated dense Q8 prefill GEMMs about 3% faster and was bit-identical to the existing half-MMA output, but maintaining a safe f32 residual/normalized vector plus half mirror added 2 MiB at maxB=512, an extra write, and a barrier. Matched-thermal pp512 was neutral. General half output storage would be lossy and require every consumer. Keep f32 activations and the existing targeted packed-MoE/head precision tiers. |
...
| 11d. Fixed-shape IQ3_XXS decode gate/up | validated | `moegucmp` reports zero bit mismatches across all 4,608 routed-plus-shared hidden outputs on real layer-0 weights. Full `moe` matches the CPU reference; scalar prefill remains 36/36 at 2.5e-6, dncmp/block/ablock pass, the mandated decode is exact, eight slots are identical, and handoff is exact. | Specializing K=2048/n_ff=512 makes each lane's two 32-element groups compile-time fixed and removes row division: gate/up fell from 29.8-29.9 to 29.2-29.6 us/layer, about 1-2%. Across six 200-token single-command-buffer runs, rollback GPU time averaged 1742.1 ms versus 1738.0 ms fixed, about 20 us/token or +0.24%; pairwise movement was thermal-noise-sized but matched the 12-24 us/token stage budget. The same specialization on 80B IQ4 regressed 40.5-40.9 -> 41.7-41.8 us and was removed. Standalone IQ3 GEMV is not used by production decode, so no dead-path specialization was kept. | Default only the exact 35B IQ3 shape; `QK_MOE_GU_FIXED=0` restores the dynamic gate/up kernel. Retain the real-weight comparator; keep 80B IQ4 on its safe kernel. This commit. |
| 11e. Fixed-shape MoE down projection | hard wall | Real routed-plus-shared outputs were bit-exact for 35B IQ4, 35B Q6_K, and 80B IQ4 at K=512/M=2048. Both prototypes and their temporary comparator were removed after measurement. | IQ4 regressed from 20.9 to 21.1-21.3 us on 35B and from 24.4-24.5 to 24.9 us on 80B. Q6_K improved 30.3-30.5 -> 28.7 us, but only layers 34/38/39 use that format: the absolute ceiling is about 5 us/token, roughly 0.06% and below whole-graph resolution. | Reject both variants: the common IQ4 path loses, while a separate Q6 pipeline and duplicated dequant kernel are not justified by a sub-0.1% ceiling. Rank 11 codegen is closed after the Q8, IQ4 dense, Q6 head, and IQ3 gate/up wins. |
| 12. Active-slot physical compaction | validated | The permanent `slotcmp` gate passes all 8 logical indices on 35B and all 4 tested on 80B, exercises every physical hole plus an irregular simultaneous mask, compacts surviving recurrent/KV state, restarts heterogeneous prompts, and reproduces each independent golden stream exactly. Forced serial prompt ingestion (`QK_NO_BATCH=1`) also passes; that stronger test caught and now guards the cursor-vs-pos live-KV boundary. G0-G8, fork/cache restore, heterogeneous staggered serving, 200-token eight-slot determinism, handoff, and a local 20+20 split-stage run are green. | In a sequential sparse workload (two 200-token streams, second admitted at logical slot 1 after slot 0 drained), default was 4159.0/4167.9 ms versus 4619.9 ms rollback and 4581.5 ms pre-change baseline: about +10-11% end to end. Fully occupied B=8 was neutral at 198.8 tok/s default versus 199.1 rollback. Compaction fills each hole with one high stripe, copying fixed recurrent state plus only that slot's attention live prefix, and runs only on lifecycle changes rather than the hot step. | Default unsplit logical-to-physical packing; `QK_SLOT_COMPACT=0` restores identity stripes. Split stages retain identity mapping, preserving the production state-save/load ABI. Keep `slotcmp` as the churn regression. This commit. |
| 13. Concurrent multi-slot prefill | validated for 35B; 80B union hard wall | `prefillmulticmp` reports zero differing state floats plus exact one-token handoff for B2 N={192,256,335,511}, B4 N={255,256}, mixed bases {37,42}, tail-padded DeltaNet chunks, and packed grouped-MoE v5. Long `slotcmp` uses distinct prompts and lengths, every hole, cancel/restart, and matches independent serial goldens. The final default passes expanded scalar prefill 42/42, G0-G8, cache/fork, heterogeneous stagger, short B8 determinism, handoff, and split-stage smoke. A direct 80B union changed state immediately after layer 0 (rel-RMS about 0.9 and handoff mismatch), so IQ4/80B admission is explicitly kept serial; default and rollback 80B long-prompt streams are identical. | Isolated exact-class GPU time: B2 N192 627.4 -> 488.5 ms (1.28x), B2 N256 about 733 -> 607 ms (1.21x), B4 N255 1489.4 -> 1081.9 ms (1.38x), and B2 N511 1224.8 -> 1084.6 ms (1.13x). Packed v5 B2 N256 was 427.5 -> 323.5 ms (1.32x). Thermally bracketed 336-token end-to-end admission+20 decode was rollback 1237.1/1238.4 vs coalesced 1115.5/1102.2 ms at B2 (+11-12%) and rollback 4260.1/4299.1 vs 3755.9/3760.9 ms at B8 (+13-14%); all streams were exact. Single pp512 remains 322.84 ms (1586 tok/s), and short/steady decode is unchanged. | Default deferred/coalesced admission only for unsplit 35B with at least two slots; `QK_PREFILL_MULTI=0` restores eager serial admission. Flatten stateless rows for dense GEMM/MoE, but issue existing attention/DeltaNet kernels per sequence on buffer slices in the same command buffer. Preserve each sequence's MoE regime, avoid 16-191-token tails, and use the 1024-row arena only for eligible multi-slot 35B engines. Keep 80B and split stages on the lean 128-row serial path. This commit. |
| 14. Decode DeltaNet state-update retile | validated; f32 MMA hard wall | Permanent `dnstepcmp` compares conv state, all 2 MiB/head-state buffers, and gated output bit-for-bit for B={1,8} and kDiv={0,2}; the retained fused-cache, split-cache, and ordered row-SIMD schedules have zero differing floats. Scalar prefill is 42/42 at 2.8e-6 worst relative drift; dncmp, recurrent/attention blocks, the mandated 20-token continuation, 200-token B8 determinism, handoff, cache restore, all speculative corruption modes, heterogeneous serving, every compaction transition, and split-stage smoke are green. Final 80B B8 is deterministic and its N128 handoff is exact. | At B1, the split 32-v-head/cache8 chain is about 11-12 us/layer versus 21-33 us for the fused streamed mapping in isolation; a full 35B 160-token bracket improved GPU time 1389.8 -> 1351.6 ms (+2.8%) and wall 1449.5 -> 1417.3 ms (+2.2%). At B8, ordered row-SIMD is about 43 us/layer versus 51-59 us fused; 35B wall time fell 4693.6 -> 4658.7 ms (+0.75%), while 80B B1 fell 1336.7 -> 1305.8 ms (+2.3%) and final B8 3602.1 -> 3523.2 ms (+2.2%). Cache-width full-block sweep selected 8 float4s (4/8/12/16 panels: 186.4/182.1/186.0/192.8 us). The 8x8 f32-MMA panel wasted seven replicated result columns and cost 26.1 us at B1 versus 11.7 split-cache, and 82.5 us at B8 versus 61.5; it was removed. | Default by compact active count: split conv/norm + cache8 state for B<=4; fused cache8 for B=5-6; split prep + one-read ordered row-SIMD for B>=7. The split restores 32 independent v-head TGs without reintroducing the shared-q/k conv race; ordered shuffles retain the original left-to-right arithmetic. `QK_DN_DECODE=0` restores the old fused streamed kernel; diagnostic `QK_DN_DECODE_SPLIT/RES` overrides remain. Losing panel/MMA/reassociated-reduction kernels were removed. This commit. |
| 15. Adaptive split-K decode attention | validated; merged-softmax and atomic completion hard walls | Permanent `fasrvcmp` compares scalar, shipping split, compact live work, and fan={1,4,8,16,32,64} coarsening under the real concurrent encoder. Every retained producer is bit-for-bit identical to shipping split (`split_diff=0`) for B={1,8}, linear and bimodal slot lengths, 256-key boundaries through 32K, and a 2,000-iteration B8/8K concurrent stress. The final 35B matrix is green: scalar prefill 42/42 at 2.8e-6, dncmp/block/ablock PASS, mandated decode exact, B8/200 deterministic, handoff exact, cache/rollback/slot-churn/heterogeneous/split-stage gates exact. 80B B8 short serving is deterministic, N128 handoff is exact, and three consecutive candidate B8/8K processes reproduced control hash `749f04982903cdcf` on every slot plus `264 11012 13721 85624 1881 1330 22767 11`. One earlier first-touch candidate process produced a different trajectory; neither repeated control/candidate runs nor the stress gate reproduced it, and no retained attention output differed from shipping split. | Exact producer coarsening lowered isolated B8/8K attention from 4.25 to 1.94 ms/layer (2.18x). Real 35B GPU ms/step fell 48.31/48.55 -> 41.73/41.92 at B8/4K (+15.8% throughput), 76.05/75.66 -> 51.34/51.25 at B8/8K (+47.9%), and 25.87 -> 18.60 at B1/32K (+39.0%). Linear B8 4K..8K fell 62.61/62.70 -> 55.05/54.96 (+14.0%); three-deep and four-deep 8K bimodal cases gained about 2%, while two-deep/16K was neutral and stays near the crossover. 80B B8/8K fell 99.49-99.64 -> 69.62-70.03 ms/step (+42-43% throughput) with the exact stream. A compact one-chunk work list saves about 5% in isolated one-deep/seven-short attention but only about 0.3% end to end. A fan-2 short-context isolated win was neutral/slower in full decode and was removed. | Default split attention now chooses a balanced producer fan targeting about 16 live groups, retains the rectangular shipping grid for one/few short streams, and compacts only grids with over 75% empty work. Each coarsened TG amortizes q loading/scheduling but writes the original independent 256-key partials; the unchanged reducer preserves arithmetic exactly. `QK_FA_SPLIT=0` restores scalar attention, `QK_FA_WORK=0` restores shipping split scheduling, and `QK_FA_FAN=1` forces the original fan. Merged partials, tail-balanced merged softmax, and atomic completion were removed. This commit. |
| 16. Router/select completion fusion | hard wall: nondeterministic cross-TG visibility | A temporary `routerselcmp` harness was bit-exact for 256/512 experts, B={1,2,8}, NSG={4,8,16}, repeated counter reuse, and all-zero stable ties. Exact 35B and initial 80B serving streams also passed. The decisive interleaved 80B B2 `fadecode` run then reproduced control hash `2ca4799acd02a858` in both controls and the first candidate, but a second identical candidate produced hash `e18aa1099c5d1e83` and repeated token 854. That is a hard correctness failure, not acceptable timing noise. | Synthetic 256-expert router+select improved 30.85 -> 17.42 us at B1 and 72.09 -> 46.78 us at B8. At 512 experts, the practical NSG8 fusion was only 26.49 -> 25.79 us at B1 and regressed 32.28 -> 35.15 us at B2 and 83.45 -> 85.62 us at B8. Full 35B was neutral (831.0-831.2 ms/100 tokens control versus 829.6 candidate at B1; B8 neutral/slower). 80B wall time suggested about 1.2% at B1, 1.0% at B2, and no B8 gain. The decisive B2 GPU A/B/B/A was 16.011/16.033 ms controls versus 15.765/15.932 ms candidates, but the latter candidate was corrupt. | Remove the kernel, counter buffer, host wiring, and temporary harness. Available Metal atomics are relaxed and explicit device fences did not establish reliable ordinary-store visibility to the last threadgroup. The safe explicit `memoryBarrierWithScope` plus separate stable selector remains; no sub-2% gain justifies an undocumented race. This commit. |
| 17. Residual/normalization representation and fusion | validated for B>=8 router; other forms hard walls | Permanent `normroutecmp` reproduces raw residual, normalized activation, and every router logit bit-for-bit for 256/512 experts and B={1,2,4,8}; the separate norm uses exactly the fused kernel's e=0 simdgroup reduction and keeps scale outside the router dot. Full B={2,4,5,6,7,8,16} trajectories matched rollback hashes. Final 35B G0-G7 is green: scalar prefill 42/42 at 2.8e-6, dncmp/block/ablock PASS, mandated decode exact, B8/200 deterministic, and handoff exact. Final 80B B8/100 is deterministic. | At B8, isolated fused/split router stages were about 40.0/28.5 us for 256 experts (1.4x) and 74.5/46.7 us for 512 experts (1.6x). Full 35B B8 GPU time fell 33.613/33.580 -> 33.355/33.343 ms/step (+0.7% throughput), and B16 58.749 -> 58.117 (+1.1%). Full 80B B8 fell 47.625/47.747 -> 46.684/46.598 (+2.2-2.5%). The extra barrier loses below saturation: 35B B2 12.035/12.069 -> 12.244/12.237, B4 loses about 0.45%, B5 loses, B6/B7 are neutral; 80B B1 10.829/11.002 -> 11.325/11.350. Exact cached-tail variants were isolated-only: pp512 controls 321.64/322.85 ms versus cached 322.42/321.84, while 160-token decode controls 1356.2/1350.9 ms GPU versus cached 1342.8/1355.3. | For eight or more active slots, compute residual/RMS once in `add_rmsnorm_sg`, persist one scale/slot, and run `moe_logits_scaled`; otherwise retain fused `moe_logits_addn`. `QK_MOE_NORM_SPLIT=0/1` forces either side. Engines configured below eight slots create neither extra pipeline nor scale buffer. Deferred raw+inv-RMS consumption was rejected by the lifetime/byte audit: all three next-layer projections would reread norm weights and add multiplies, eliminating the one normalized-vector write saving; grouped prefill already computes normalization once before router GEMM. Cached tails, down-tail fusion, and atomic-last-TG forms remain removed. This commit. |
| 18. GPU-resident multi-step command encoding / ICB | hard wall at EOS-safe value | A temporary serving path precomputed per-step position rows, wrote each argmax to a token ring, fed the preceding ring slice directly to the next embedding, and encoded K dependent full-model steps in one concurrent command buffer. With `QK_NO_EOS=1`, K={2,8}, B={1,8}, and both models reproduced the serial token stream exactly. The implementation was removed. | 35B B1 K8 wall time was 1711.8/1709.5 ms control versus 1662.6/1662.2 ms batched (+2.9%); production-like K2 was 2099.5/2104.6 versus 2079.7/2085.0 (+0.9%). At B8/K8 it was 4635.5/4632.0 versus 4562.8/4595.4 (+0.8-1.6%). For 80B B1/K2, the stable control was 1503.0 ms and candidates 1503.4/1492.2 (0-0.7%); the 1538.5 first control was a cold outlier. | Reject general integration. An EOS at interior step j is known only after the command buffer has already mutated recurrent state through K; discarding that state preserves output but silently destroys the finish-time prefix-cache snapshot. Exact recovery needs copying roughly 62.8 MiB of recurrent state before every chunk and replaying through j, or adding a live mask to every state-mutating kernel. Both exceed the sub-1% K2 budget. Normal token mode already encodes the whole generation in one command buffer; serving retains per-step waits. This commit. |
| 19. Activation/scratch arena aliasing and lazy allocation | validated | A phase-lifetime coloring shares whole offset-zero Metal buffers across decode and prefill without changing shader bindings. Full 35B G0-G7 is green: scalar prefill 42/42 at 2.8e-6, dncmp/block/ablock PASS, mandated decode exact, B8/200 and B16 deterministic, and handoff exact. Additional exact gates cover maxB=1024 coalesced B2x512, padded B2x335 with bases, grouped-MoE v3/v5, `QK_DN_CHUNK=0`, cache restore, heterogeneous admission, every slot-compaction hole, split stages, 1K/8K split-attention control hashes, and all speculative rejection modes. Final 80B B8 is deterministic and N128 handoff is exact. | At 35B B8/maxB1024, managed scratch falls 400.5 -> 202.8 MiB; B16 falls 431.9 -> 225.2 MiB. Measured `peak memory footprint` fell 227.0 MiB between the final layout and independent buffers plus the old speculative stripe; recurrent-only carry and attention-only MMA slack remove another 11.25 MiB, for about 238 MiB total versus the prior layout. At 80B B8/maxB128, aliases save 30.1 MiB, the lazy recurrent stripe saves 75.4 MiB, and carry/slack sizing saves another 13.5 MiB (about 119 MiB total). pp512 controls 323.29/322.53 ms versus aliases 322.37/322.26 ms. Decode was neutral: B1 9.392/9.388, B8 32.831/32.766, and B8/8K 50.620/50.468 ms/step control/alias, with identical hashes. | Keep exact phase coloring by default; `QK_SCRATCH_ALIAS=0` restores independent activation allocations and `QK_MEM_STATS=1` reports the ledger. Decode split-attention partials share storage with decode logits and prefill carry/head candidates across non-overlapping API modes; prefill projection, DN, MoE, and head phases reuse ten explicitly separated groups. Compact conv carry to owned recurrent layers, reserve MMA overread slack only on attention K/V, and allocate the recurrent verification stripe only for `QK_SPEC` (the direct `speccmp` harness requests it explicitly). Full-logit/top-k behavior and every live state stride remain unchanged. This commit. |
| 20. Prefix-cache snapshot compaction | validated | Permanent `cachecmp` compares every restored byte with the original full-stripe copier at lengths {8,17,64,127,255,384}, long-to-short and short-to-long overwrite, a 65-token restore from a stored 384-token prefix, rejected over-restore, `n_tok=0`, all six entries, and LRU eviction. Packed and full rollback layouts pass on 35B and 80B; first, middle, and tail split stages are exact. `cachetest` warm output equals cold output. Final 35B G0-G7, `slotcmp` with six entries, and 80B B8 determinism/N128 handoff are green. | At production-shaped 80B/49,152 context, one 21-token snapshot falls from 2,379.4 to 76.4 MiB and measured peak footprint falls 2.25 GiB; six such entries require about 458 MiB instead of 13.9 GiB. A snapshot-bearing one-token request fell from 665.7 to 525.9 ms. At 35B/512, the six-entry mixed-length test falls from 496.9 to 379.8 MiB; packed cache restore remains about 2.0x faster than cold prefill. Payload grows as fixed recurrent state plus live K/V and converges to the full size only at maximum context. | Default lazy exact-size snapshots: full recurrent stripes plus per-head live K/V, with exact resize on overwrite and unchanged GPU state layout and public save/load signatures. Empty entries allocate no payload. `QK_PCACHE_COMPACT=0` restores lazy full-layout snapshots; `QK_MEM_STATS=1` reports full capacity and each save. Keep the original full copier as the regression oracle and `cachecmp` as the permanent state/LRU gate. This commit. |
| 21. Optional f16 KV storage for capacity | validated precision tier | Typed `facmp` covers N={1,7,9,17,65,127,128,512,1024}, nonzero bases through 512, and near-capacity tails; f16 GQA stays within 4.5e-8 of the scalar-f16 reference. `fasrvcmp` is bit-exact between scalar and every split/compact/fan schedule through B8/8K. The scalar-projection prefill sweep is 42/42 argmax-consistent (1.1e-3 worst relative logit drift), serial and batched 1,040-token admission produce the same continuation, handoff is exact, and B8/200 is deterministic. Packed/full cache layouts, split-stage snapshots, slot churn, all speculative rollback modes, and 35B/80B handoff and serving gates pass. Default f32 retains the full token-exact G0-G7 matrix and final 80B checks. | At B2/49,152, managed peak falls by 1.878 GiB on 35B and 2.253 GiB on 80B; full snapshot capacity falls 1.94 -> 1.00 GiB and 2.32 -> 1.20 GiB respectively. Half-to-f32 GQA is 3.2-4.5x faster than scalar-f16 attention, but thermally bracketed pp512 is about 2.4% slower than f32 (330.17 vs 322.24-323.04 ms), and full 8K prompt time is about 12.4% slower (25.14 vs 22.36 s); deep decode itself is neutral, matching the historic no-speed-win result. | Keep strictly behind `QK_KV_F16=1`; f32 remains the default. Every KV writer/reader, allocation/stripe, active-slot move, and packed/full snapshot uses one compile-time-specialized width while attention accumulates in f32. This tier is for context/cache capacity only and accepts numerical drift plus possible long-context greedy divergence; it is ineligible for the token-exact default. This commit. |
| 22. Reduced-precision DeltaNet/activation storage | hard wall: state drift/slow exact path; activation mirror neutral | Half state with f32 accumulation preserved the short 40-token stream, B8/200 determinism, exact N128 handoff, and the f32 1,040-token-plus-100 continuation; B1/8K also matched the f32 hash. The isolated same-rounded-state output was exact for one step, then rounding became visible by step 8. Scalar prefill remained 42/42 argmax-consistent, but serial-versus-chunked logits reached 0.4865 max absolute and 5% worst relative drift. Half-X dense GEMM input reproduced its f32-input half-MMA twin bit-for-bit. | Half state saves 30 MiB/slot on 35B and 36 MiB/slot on 80B (180/216 MiB across six snapshots). The B8 row-state kernel improved 5.6-9.2%, but the B1 half row kernel remained slower than the shipping split8 schedule, fast chunked pp512 showed no win, and exact sequential state rounding cost 393.59 ms versus about 333 ms (+18%); isolated sequential DeltaNet was 2,780 us versus 657 us chunked (4.23x). Half-X improved isolated 8192x2048x512 and 2048x4096x512 Q8 GEMMs about 3%, but the integrated f32+half mirror was neutral in the closest thermal pair (333.08 versus 334.98 ms) and added 2 MiB at maxB=512. | Remove every prototype. The recurrent capacity saving is too small for path-dependent precision drift, and making rounding serial-exact destroys pp512. A safe activation mirror's extra stores/barrier erase the kernel gain; half output storage would be a consumer-wide lossy tier already better targeted by packed MoE/head paths. Keep f32 recurrent state and activations. This commit. |
| 23. Row-Q8 KV and GQA decode reuse | validated precision tier | Synthetic prefill and serving references measure about 0.65-0.75% relative-RMS Q8-versus-f32 drift; the retained Q8 GQA schedules stay within 6e-8 of scalar-Q8. Scalar-versus-batched prefill is 42/42 argmax-consistent (8.3% worst relative logit drift). Serial/batched handoff, packed/full cache restore, slot compaction/churn, heterogeneous serving, every speculative rollback mode, 35B B8/200, and 80B serving are self-consistent. The 8k continuation still matches f32; at 32k the coherent Q8 stream can diverge after token 32, so this is explicitly not a token-exact tier. The final default f32 G0-G7 matrix is green. | One aligned record is 272 bytes per 256 values (1.0625 B/value), 3.76x smaller than f32. At B2/49,152 this saves about 2.758 GiB of managed 35B KV and 3.309 GiB on 80B; full packed-snapshot capacity is 0.56/0.67 GiB. Full 35B B1/32k fell 20.771 -> 10.918 ms/step (1.90x), B8/32k 139.031 -> 47.075/47.188 ms/step (2.95x aggregate), B8/8k about 51.25 -> 36.124 ms/step (1.42x), and a pre-GQA B1/49k result was 31.056 -> 17.021 ms/step. pp512 regresses about 2% (f32 326.9-327.3 ms, Q8 332.5-335.7 ms). | Keep behind `QK_KV_Q8=1`; f32 remains default. Use an f32 scale at byte 0, signed data at byte 16, vectorized char4 dequant, and adaptive 1/2/4/8-query-head reuse without changing each head's softmax/partial structure. Unaligned 260/264-byte rows were slower, and coarsened Q8 chunk producers lost at B1 and B8; both were removed. This commit. |
| 21b. Half-KV grouped-query decode reuse | validated precision-tier refinement | `fasrvcmp` shows zero `split_diff` for HG={2,4,8} across 1-8 slots, full and 1/7-key tail chunks, balanced contexts through 32k, and heterogeneous 31..8192 positions. Candidate and rollback hashes match at B1/B8 and 8k/32k. Final f16 token, B8/200, handoff, cache, slot-churn, heterogeneous-serving, and every speculative rollback gate pass on 35B; 80B B8/handoff pass. Q8 regression checks and the final default-f32 G0-G7 matrix are green. | Isolated B1/32k attention fell 844 -> 291 us/layer and B8/32k 9,199 -> 1,781 us/layer. Full 35B B1/8k fell 11.229 -> 9.600/9.517 ms/step (+17-18% throughput), B8/8k 53.250/53.357 -> 37.369 (+43%), and B1/32k 18.272 -> 11.549/11.550 (+58%). A conservative cool B8/32k control fell 110.105 -> 49.436 ms/step (2.23x); a later 150.366-ms control was thermally degraded. B1 short context was neutral (9.301-9.311 vs 9.321), as was the stable B8 short bracket. Prefill is untouched. | Within `QK_KV_F16=1`, default to one split producer per 256-key chunk and share half K/V across adaptive HG2/HG4/HG8; use HG8 only for balanced batches with at least 128 live chunks and retain compact generic work for severely sparse batches. `QK_FA_F16_GQA=0` restores the prior bit-exact schedule. This commit. |
| 15b. Exact f32 grouped-query decode reuse | validated | Every HG={2,4,8} output is bit-for-bit identical to the shipping split producer for 1-8 slots, 1/7-key tails, 128/256-key boundaries, balanced contexts through 32k, and heterogeneous 31..8192 positions. Candidate/rollback full-engine hashes match at B1/B8, 8k/32k, and 80B B8/8k. Final 35B G0-G7, cache, slot churn, heterogeneous serving, and all speculative rejection gates are green; f16/Q8 regressions and 80B serving/handoff pass. | Isolated B1/32k attention fell 1,743-1,795 -> 385-387 us/layer and B8/32k 18,353 -> 2,500 us/layer. Full 35B B1/8k fell 11.394/11.370 -> 9.793 ms/step (+16%), B8/8k 57.518/56.682 -> 39.564 (+43%), and B1/32k 21.867 -> 14.636 (+49%). In the thermally soaked B8/32k bracket, 165.028/162.469 -> 77.687 ms/step (2.09-2.12x); short B1/B8 remained noise-flat. 80B B8/8k fell 74.390/74.899 -> 56.602 ms/step (+32%). Prefill is untouched. | Default exact f32 split decode to adaptive HG2/HG4/HG8, using HG8 from 128 live chunks and retaining the compact generic work list only for severely sparse grids. Each head keeps its original score order, softmax, partial slot, and unchanged reducer. `QK_FA_F32_GQA=0` restores the prior bit-exact adaptive fan schedule. This commit. |
| 24. Optional K-major Q8 prefill weights | validated opt-in; other formats/cache hard walls | The K-major kernel is bit-for-bit identical to the established aligned Q8 GEMM for all four real M/K families, including 4,194,304 outputs at 8192x2048x512. Final 35B scalar prefill is 42/42 at 2.8e-6; dncmp/block/ablock, required decode, B8/200, handoff, B2 coalesced prefill state/handoff, forced four-window open, staggered serving, and cache restore all pass. 80B explicitly rejects the Q8-only option and opens normally. | The 1.27-GiB arena packs in 126-147 ms. Real-shape dense GEMMs improve about 4-7%; projection-only pp512 falls 121.96/122.07 -> 118.35/116.82 ms. Matched-memory five-repeat pp512 falls 324.34/325.46 -> 321.61/322.86 ms (+0.5-1.2%). Warm source/PSO work is only 7.05 ms total. K-major Q8 decode loses 4-35%; IQ4 prefill is neutral. | Keep default-off behind `QK_PREFILL_REPACK=1`; `QK_PREFILL_REPACK_USE=0` is the matched-memory dispatch rollback. Native rows remain authoritative for decode and non-32 tails. Reject an IQ4 twin, shared decode layout, binary archive, and persistent sidecar at the measured EV. This commit. |
| 25a. Exact four-row Q8 tail prefill | validated; mixed-format initialization corrected | Fixed-K B4 kernels are bit-for-bit identical to the shipping fixed GEMV on every real dense shape (K=2048, M={512,4096,8192}, TPR=16; K=4096, M=2048, TPR=128). Final scalar prefill is 42/42 at 2.8e-6; N={16,128} handoff is exact; dncmp/block/ablock PASS; mandated decode is exact; and B8/200 is deterministic. A final 80B audit exposed that its mostly-IQ4 graph retains a Q8 attention-V projection: the selector reached an uninitialized B4 pipeline on 21-row prompts. Initializing the Q8 pair for mixed-format engines restores 80B B8/200 determinism, exact N={16,128} handoff, and a 40-token stream identical to `QK_PREFILL_GEMV_BATCH=0`. | Full-model tail chunks improve from 32.81 to 30.22 ms at N=8, 57.95/58.52 to 54.70/54.38 ms at N=16 (+6-8%), and 111.86/111.60 to 107.06/105.90 ms at N=32 (+4-5%). Isolated B4 reuse is 1.15-1.41x over grid-z across real shapes. Lowering the half-MMA crossover from 48 to 16 was much faster (N16 56.9 -> 46.8 ms) but failed exact handoff after three tokens for seed 1234. The fixed B2 specialization also changed bits; B8 was slower at N32. | Keep the half-MMA crossover at 48. Below it, default only the fixed-K four-row reuse kernel on the exact production TPR pair, with a 1-3-row shipping-GEMV tail; initialize those Q8 pipelines even in the mixed IQ4/Q8 80B. `QK_PREFILL_GEMV_BATCH=0` is the rollback. Remove the lossy/losing alternatives. Corrected in a follow-up commit. |
| 25b. Exact-class grouped-MoE crossover | reverted by final audit | The original candidate check forced grouped v4 at every N, so both the serial-admission N=127 side and direct N=128 side used the same regime and passed. The committed threshold instead grouped only N=128; five repeated default G7 runs deterministically diverged for seed 99 after four tokens. `QK_MOE_GROUP_N=192` restores exact handoff. N=96 was already rejected after a seed-99 divergence. | Natural-routing full batches improve from 210.15 to 199.74 ms at N=96 (+5%, rejected) and 276.49 to 234.25 ms at N=128 (+15%, rejected). The old maxB128 long-prompt configuration improved 2504.8/2512.0 to 2260.0/2250.8 ms, but rank 25c now admits the same prompt in a 1,024-row chunk that remains grouped even with the safe 192 threshold. pp512 is also unchanged because 512 >= 192. | Restore the default threshold to 192. A crossover keyed only to the current batch length cannot safely straddle `qk_slot_start`'s intentional N-1 admission boundary. Keep explicit grouped variants for precision-tier/performance experiments, but never trade the required handoff gate for this local win. Corrective commit after `941e9a8`. |
