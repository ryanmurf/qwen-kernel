...
**Progress:** Small-batch dense, grouped-MoE, and whole-prompt chunk crossovers
are closed by results 25a-25c below. DeltaNet chunk geometry remains active.
...
| 25a. Exact four-row Q8 tail prefill | validated | Fixed-K B4 kernels are bit-for-bit identical to the shipping fixed GEMV on every real dense shape (K=2048, M={512,4096,8192}, TPR=16; K=4096, M=2048, TPR=128). Final scalar prefill is 42/42 at 2.8e-6; N={16,128} handoff is exact; dncmp/block/ablock PASS; mandated decode is exact; and B8/200 is deterministic. The IQ4/80B path is not selected. | Full-model tail chunks improve from 32.81 to 30.22 ms at N=8, 57.95/58.52 to 54.70/54.38 ms at N=16 (+6-8%), and 111.86/111.60 to 107.06/105.90 ms at N=32 (+4-5%). Isolated B4 reuse is 1.15-1.41x over grid-z across real shapes. Lowering the half-MMA crossover from 48 to 16 was much faster (N16 56.9 -> 46.8 ms) but failed exact handoff after three tokens for seed 1234. The fixed B2 specialization also changed bits; B8 was slower at N32. | Keep the half-MMA crossover at 48. Below it, default only the fixed-K four-row reuse kernel on the exact production TPR pair, with a 1-3-row shipping-GEMV tail; `QK_PREFILL_GEMV_BATCH=0` is the rollback. Remove the lossy/losing alternatives. This commit. |
| 25b. Exact-class grouped-MoE crossover | validated at N>=128; N=96 rejected | Scalar prefill remains 42/42 at 2.8e-6; N=128 three-seed handoff is exact; the 1,040-token natural prompt plus 64-token continuation exactly matches `tests/ref4.txt`; dncmp/block/ablock PASS; mandated decode is exact; and B8/200 is deterministic. N=96 grouped v4 failed seed-99 handoff after two matching tokens and is ineligible despite matching immediate argmax. | Natural-routing full batches improve from 210.15 to 199.74 ms at N=96 (+5%, rejected) and 276.49 to 234.25 ms at N=128 (+15%). More importantly, the production single-stream 1,040-token admission at maxB=128 falls from 2504.8/2512.0 to 2260.0/2250.8 ms (+11.1-11.6%) with the exact 64-token continuation. N=64 loses 143.72 -> 159.68 ms; N=80 loses 181.10 -> 191.14; N=88 ties 196.42/196.17. | Lower the default exact-class v4 threshold from 192 to 128 and retain `QK_MOE_GROUP_N=192` as rollback. Do not chase the faster numerical crossover below the first exact handoff boundary. Packed v5 remains an explicit lossy tier even though it wins earlier. This commit. |
| 25c. Unsplit-35B prefill chunk capacity | validated | Default 1,024-row admission reproduces all 64 tokens in `tests/ref4.txt` after the 1,040-token prompt. Short B1/200 remains exact; the final G0-G7 matrix from 25b applies because two-slot/required gates already used the unchanged 1,024-row arena. Split engines and IQ4/80B remain at 128. Flattened capacity is clamped to `nCtx*nSlots`, so a single short-context engine does not allocate unreachable rows. | Single-stream 1,040-token admission sweep: maxB 128/192/256/384/512/768/1024 = 2258/1884/1760/1510/1481/1379/1329 ms. A reverse repeat was 1,356 vs 2,258 ms: 1.67-1.70x throughput at 1024. Aliased scratch rises 26.2 -> 195.4 MiB versus maxB128 (+169 MiB); at ctx512 it clamps to 512 rows. | Default unsplit Q8/35B engines to maxB=1024 regardless of slot count. Retain 128 for split or IQ4/80B paths and `QK_MAXB` for explicit control. The 169-MiB capacity cost is well below the 15-GiB model and buys a dominant long-prompt win. This commit. |
