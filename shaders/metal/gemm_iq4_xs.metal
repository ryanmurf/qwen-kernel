...
// Full-tile specialization selected only for M%64==0 and N%32==0.  It keeps
// the safe kernel's exact decode/MMA order while removing all hot-loop bounds
// predicates and zero-fill branches.
kernel void gemm_iq4_xs_hp_aligned(device const block_iq4_xs* wb [[buffer(0)]],
                                   device const float*        x  [[buffer(1)]],
                                   device float*              y  [[buffer(2)]],
                                   constant GemmPC&           pc [[buffer(3)]],
                                   uint3 tid3  [[thread_position_in_threadgroup]],
