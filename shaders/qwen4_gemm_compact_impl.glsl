// Opt-in Halo compact tile: BK32, activation reuse across eight rows.
// Baseline shader remains separate. Keep each 32-element F32 dot and the
// subsequent block accumulation order; GPU/model parity must gate deployment.
...
#define TUNE_REUSE 0
#endif
const uint BK = TUNE_BK, SK = BK + 1u;
...
void main() {
    ...
    for (uint b0 = 0u; b0 < kb; b0 += BK / 32u) {
        ...
        [[unroll]] for (uint bb = 0u; bb < BK / 32u; ++bb) {
#if TUNE_REUSE
            float sums[RM][RN];
            [[unroll]] for (uint mi = 0u; mi < RM; ++mi)
                [[unroll]] for (uint j = 0u; j < RN; ++j) sums[mi][j] = 0.0;
            [[unroll]] for (uint i = 0u; i < 32u; ++i) {
                float xv[RN];
                [[unroll]] for (uint j = 0u; j < RN; ++j)
                    xv[j] = Xsh[(tc * RN + j) * SK + bb * 32u + i];
                [[unroll]] for (uint mi = 0u; mi < RM; ++mi) {
                    float wv = Wsh[(tr * RM + mi) * SK + bb * 32u + i];
                    [[unroll]] for (uint j = 0u; j < RN; ++j)
                        sums[mi][j] += wv * xv[j];
                }
            }
