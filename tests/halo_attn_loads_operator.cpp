...
int main(int argc,char** argv) {
    ...
    bool pass=true;
    {
        ...
        Kernel kvec4(c,"kvec4.spv",{&q,&k,&v,&gates,&output,&position},32);
        Kernel v8(c,"v8.spv",{&q,&k,&v,&gates,&output,&position},32);
        ...
        for(const Case test:std::vector<Case>{{1},{2},{63},{64},{65},{255},{256},{257},
            ...
            for(uint32_t mode:{0u,1u,2u,3u,3u,2u,1u,0u}) {
                Kernel& kernel=mode==0?base:(mode==1?kvec4:(mode==2?v8:kv8));
                ...
                pass&=check(test,output.ptr,original,gold,poison,
                           mode==0?"baseline":(mode==1?"kvec4":(mode==2?"v8":"kv8")),order++,us);
