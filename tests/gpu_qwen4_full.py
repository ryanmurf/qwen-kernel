...
def main():
    ...
    try:
        ...
        token = (u32*1)(args.token)
        assert lib.qk_stage_run(engines[0],0,token,None,1,0,hidden,None)==0
        assert lib.qk_stage_run(engines[1],0,None,hidden,1,0,None,ids)==0
        assert lib.qk_stage_logits(engines[1],logits,len(logits))==0
        assert list(logits)==first, "full-model reset changed logits"
        assert all(r["split_relative_rms"]<1e-5 and r["logit_relative_rms"]<1e-5
                   and r["native_argmax"]==r["reference_argmax"] for r in records), "full-model parity failed"
        print(json.dumps({"full_model_parity":"PASS","frames":args.steps,"reset_exact":True}),flush=True)
