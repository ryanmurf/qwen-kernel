...
// c++ -O2 -std=c++17 -pthread tests/halo_expert_layout.cpp -lvulkan -o OUT
#define QK_LIBRARY
    ...
    try {
        ...
        for(uint32_t layer : {0u,47u}) {
            ...
            for(const Case test : {Case{32,1,1},Case{32,8,1},Case{32,16,1},Case{32,17,1},Case{1,64,8},Case{16,3,.25f}}) {
                ...
                Kernel words(c,"words.spv",{&g,&u,&x,&offsets,&pairs,&out},16);
                Kernel soa(c,"soa.spv",{&gs,&us,&x,&offsets,&pairs,&out},16);
                GemmPC pc{N,F,E,U};
                std::vector<float> reference;
                uint32_t order=0;
                for(uint32_t mode : {0u,1u,2u,2u,1u,0u}) {
                    Kernel& kernel=mode==0?baseline:mode==1?words:soa;
                    const char* name=mode==0?"baseline":mode==1?"words":"soa";
                    const float poison=std::numeric_limits<float>::quiet_NaN();
                    std::fill(out.ptr,out.ptr+outputFloats,poison);
                    timed(c,1,[&]{kernel.dispatch(pc,F/128,E);});
                    const double usTime=timed(c,4,[&]{kernel.dispatch(pc,F/128,E);});
